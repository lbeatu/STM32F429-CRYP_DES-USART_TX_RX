...
{		
		//UART-serial port take char function	
    if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
		{		
				TIM_ITConfig(TIM2, TIM_IT_Update, ENABLE);
				giris[countergiris++]=USART_ReceiveData(USART1)-'0';				
...
void TIM2_IRQHandler()
	{
		...
		//data is coming from uart.wait data
		//buffer check null& !null
...
{	
	...
	// Enable clock for TIM2,UART,PORTA,PORTG
    ...
    GPIO_PinAFConfig(GPIOA, GPIO_PinSource10, GPIO_AF_USART1);
 	//UART COMMUNICATION CONFIGURATION -115200 BAUDRATE TX-RX SERIAL COM
    ...
    USART_InitStrutcure.USART_Mode                  = USART_Mode_Tx | USART_Mode_Rx;
 	//ENABLE UART ÝNTERRUPT RX-MODE
    USART_Init(USART1, &USART_InitStrutcure);
    USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);
    USART_Cmd(USART1, ENABLE);
 	//ENABLE UART ÝNTERRUPT RX-MODE- USART1_IRQn()
	...
	NVIC_Init(&NVIC_InitStruct);
		...
		uint64_t girilen=0;//0x1123456789ABCDEE; //DES-ýnput 	
		uint64_t key = 0x0000456789ABCDEF; // DES-key
		uint64_t sonuc=0; //DES-result
		int i=0;	
		char out[128]="";
		while(1)
		{	
			if(receviekey==1)
				{
						...
						{
							if (i%2 == 0) {
									sonuc = des(girilen, key,'e');
									sprintf (out,"\nDES sifresi(ecrypt): %llx", sonuc);//DES Algotrim- ecrypt input on DES cyprt
									USART_SendString(USART1, out);
            
							}else {            
									sonuc = des(girilen, key, 'd');
									sprintf (out,"\nDES decrypt : %llx", sonuc);
									USART_SendString(USART1, out);            
							}	
						}
				sonuc = des(girilen, key,'d');
				sprintf (out,"\nDES sifre Çözümlemesi(Decrypt)x16: %llx", sonuc);//DES Algotrim- Decrypt input on DES cyprt
				USART_SendString(USART1, out);
				/*
				sprintf (out,"\nGirilen Text1: %llx",girilen);//hex kod seklinde yazdirma
				USART_SendString(USART1, out);				
				sprintf (out,"\nDES sifresi: %llx", sonuc);//hex kod seklinde yazdirma
				USART_SendString(USART1, out);*/
				receviekey=0;
				}
...
{
    while(*s)
    {	//UART-serial port send string function
